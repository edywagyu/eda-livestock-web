...
(function(global) {
  ...
  const PRODUCTS_MASTER = [
    ...
    /* ===== 平飼い鶏肉セット (P048・2026-09-20 新設) =====
       鶏モモ1袋 + 鶏ムネ1袋。単品合計 ¥1,900 → ¥1,780（5.3%オフ）。
       価格の根拠：実売50注文を価格帯で見ると ¥1,500〜1,999 が注文の46%に入る最強帯。
       値引き率は「5〜7%まで」が実績（6.5%オフの肉の日セットだけが売れ、10%以上は0〜1個）。
       まとめ買いは Shopify 時代に鶏の全商品で「2つ5%オフ / 3つ10%オフ」が効いていたので同じ形。
       🧩 components(BOM) 必須。無いとセットが売れてもモモ/ムネの在庫が減らない。
       ...
       画像は 2026-09-20 に田崎さんからもらったもの（chicken-set.jpg）。 */
    {
      ...
      stripePriceId: '',
      name: '平飼い鶏肉セット', variant: '鶏モモ200g ＋ 鶏ムネ200g 計400g',
      /* stock は applyBomStock が構成品から作り直す。ここは取得失敗時の目安 */
      price: 1780, listPrice: 1900, weight: 400, stock: 0, temp: '冷凍',
      published: false,
      components: [
        { name: '平飼い鶏 モモ', qty: 1 },
        { name: '平飼い鶏 ムネ', qty: 1 }
    ...
    /* ===== 不揃い品セット（2026-09-03(木) 18:30 発売・会員限定ページの商品）=====
       和牛5種を各200g・計1kg。バラ焼肉／和牛ミンチ／赤身ステーキ／霜降スライス／切り落とし。
       🔴 2026-09-20 ryotaro指示で中身を変更（赤身スライス→霜降スライス／ステーキは赤身ステーキに固定）。
          ミスジは直近ロットの原価が¥2,781（赤身ステーキの2.2倍）で利益率が17%まで落ちるため外した。
          ...
          冷凍焼けの程度は【この商品詳細ページ】の2枚目で見せる（2026-09-02 ryotaro指示）。 */
    {
      ...
      description: '和牛5種を、各200g・計1kg。バラ焼肉／和牛ミンチ／赤身ステーキ／霜降スライス／切り落とし。スライス・バラ焼肉・切り落としは冷凍焼けで、表面の色が黒っぽく変わっています。2枚目の写真が冷凍焼けの例です。ステーキは形が不揃いです。内容量も味も、通常の商品と同じです。見た目のぶんだけお安くしました。単品で買うと ¥11,950 → ¥7,980。',
      images: [
