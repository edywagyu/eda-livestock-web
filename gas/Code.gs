...
var AUTOMATION_REGISTRY = [
  ...
  ['EC', '鶏ムネ特典の同梱もれ防止', '30分ごと', '稼働中', 'アンケートに答えてくれた人の「次に発送する注文」に🐔の印を自動でつける。管理画面の発送画面に「鶏ムネ1個（無料）を同梱してください」と赤く出て、本番EC注文DBの chicken_perk 列にも印が入る。入れたら回答シートの「鶏むね同梱」を『同梱済』に変えると30分以内に印が消える。判定はアンケート専用GAS（所有 r.tasaki@）側にあり、EC本体GASは関与しない'],
  ...
  ['社内シート', 'この一覧表の自動更新', '毎朝6時', '稼働中', 'この表をGASの登録簿から毎朝作り直す。2026-08-22に setupAutomationCatalog を実行してトリガー設置済み（以後は自動化を追加・変更すると翌朝この表に反映される）'],
  ['EC', '「今売れている人気商品」バナー', '常時（ページを開くたび）', '稼働中', 'shopページの上部に、直近14日の売れ行き上位5商品をバナーで出す。売り切れ・非公開・販売期間切れ・そのページに無い商品は自動で外す'],
  ['公式LINE', '配信・リッチメニューのクリックを「誰が」まで記録', 'お客様がリンクを押した時', '稼働中', '配信リンク(c.html)とリッチメニューのクリックを記録する。LINE連携済みの人は誰が押したかまで分かる（2026-08-21まで連携IDの取得キーが間違っており、誰が押したかは一度も残っていなかった）'],
  ['社内シート', 'クリックを「誰が・いつ・どれ」で集計', '1時間ごと', '稼働中', '記録されたクリックを本番EC注文DBの「クリック_明細」（1クリック=1行・LINE名つき）と「クリック_サマリ」（人別/リンク別/曜日別/時間帯別）に集計する。2026-08-22に installClickDashboardTrigger を実行してトリガー設置済み'],
