...
var AUTOMATION_REGISTRY = [
  ...
  ['社内シート', 'クリックを「誰が・いつ・どれ」で集計', '1時間ごと', '未設置（動いていない）', '記録されたクリックを「クリック_明細」（1クリック=1行・LINE名つき）と「クリック_サマリ」（人別/リンク別/曜日別/時間帯別）に集計する。コードは入ったがトリガー未設置（installClickDashboardTrigger を1回実行すると動き出す）']
];
