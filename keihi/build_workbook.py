...
def add_summary_sheet(wb: Workbook) -> None:
    ...
    data = [
        ["第3期分", "2024/4〜2025/3", "会議費（スタバ Mobile Order）", 30, 18638],
        ["第4期分", "2025/4〜2026/3", "旅費交通費", 64, 144644],
        ["第4期分", "2025/4〜2026/3", "通信費", 53, 135004],
        ["第4期分", "2025/4〜2026/3", "会議費（スタバ実消費分）", 16, 21657],
    ]
    for row in data:
        ws.append(row)
    ...
    explain = [
        "",
        "案A：役員借入金として処理（推奨）",
        "  （借）旅費交通費  144,644 ／（貸）役員借入金  301,305",
        "  （借）通信費      135,004",
        "  （借）会議費       21,657",
        "  → 後日、会社→代表者へ振込で精算",
        "",
        "案B：未払金として処理",
        "  （借）旅費交通費  144,644 ／（貸）未払金  301,305",
        "  （借）通信費      135,004",
        "  （借）会議費       21,657",
        "  → 後日精算時に未払金消込",
        "",
        "※ スタバカードへのチャージ（¥1,000/¥5,000ジャスト計35件 ¥43,000）は",
        "   前払金性質のため経費計上から除外しています。",
    ]
    for line in explain:
        ws.append([line])
